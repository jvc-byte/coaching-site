...
if (isset($_POST["update_profile"])) {
    ...
    if ($_FILES['profile_picture']['error'] === UPLOAD_ERR_OK) {
        ...
        $imagepath = "";
        header("location: profile.php?imgError=No image uploaded");
    }
    ...
    if (count($update_fields) > 0) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            header("location: profile.php?success=Details updated successfully!");
        } else {
            header("location: profile.php?error=Failed to update details. Please try again later.");
        }

        // Close the statement
        mysqli_stmt_close($stmt);
    } else {
        header("location: profile.php?error=No fields provided for update.");
    }
} else {
